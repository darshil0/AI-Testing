...
def generate_analytics(results_path="ai_evaluation/results/latest_results.json"):
    ...
    # 1. Score by Model
    sns.barplot(x="model_type", y="judge_score", data=df, ax=axes[0, 0], palette="viridis")
    axes[0, 0].set_title("Average Judge Score by Model")
    axes[0, 0].set_ylim(0, 1.1)

    # 2. Latency vs Score
    sns.scatterplot(x="duration_seconds", y="judge_score", hue="model_type", data=df, ax=axes[0, 1], s=100)
    ...
    sns.barplot(x="model_type", y="estimated_cost", data=cost_df, ax=axes[1, 1], palette="magma")
